...
class Mlp(nn.Module):
    ...
    def forward(self, x, H, W):
        x = self.fc1(x) if not self.is_Coord else self.fc1(x, H, W)
        x = self.act(x)
        x = self.drop(x)
        x = self.fc2(x) if not self.is_Coord else self.fc2(x, H, W)
        x = self.drop(x)
        return x
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, H, W, mask=None):
        ...
        if not self.is_Coord:
            ...
            qkv = self.qkv(x, self.window_size[0], self.window_size[1]).reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        ...
        x = self.proj(x) if not self.is_Coord else self.proj(x, self.window_size[0], self.window_size[1])
        x = self.proj_drop(x)
        return x
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=7, shift_size=0,
        ...
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop, is_Coord=is_Coord if not is_last else False)
    ...
    def forward(self, x, mask_matrix):
        ...
        # W-MSA/SW-MSA
        attn_windows = self.attn(x_windows, H, W, mask=attn_mask)  # nW*B, window_size*window_size, C
...
class BasicLayer(nn.Module):
    ...
    def __init__(self,
                 ...
                 is_last=False, is_SPT=False, is_LSA=False, is_Coord=False):
        super().__init__()
        self.window_size = window_size
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(
                ...
                is_LSA=is_LSA, is_Coord=is_Coord, is_last=True if (i == depth-1 and is_last) else False)
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(
                ...
                is_SPT=is_SPT, is_LSA=is_LSA, is_Coord=is_Coord, is_last = False if not i_layer == self.num_layers-1 else True)
    ...
    def forward(self, x):
        """Forward function."""
        x = self.patch_embed(x) if not self.is_SPT else self.patch_embed(x, x.size(2), x.size(3))
        Wh, Ww = x.size(2), x.size(3)
        if self.ape:
