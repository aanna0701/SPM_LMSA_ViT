...
class PatchShifting(nn.Module):
    def __init__(self, patch_size, is_pe=False):
        super().__init__()
    ...
    def forward(self, x):
        ...
        if self.is_pe:
            ...
            C = x.size(1)
            x_pad[:, :C//4] = x_pad[:, :C//4, :-self.shift*2, :-self.shift*2]
            x_pad[:, C//4:C//4*2] = x_pad[:, C//4:C//4*2,:-self.shift*2, self.shift*2:]
            x_pad[:, C//4*2:C//4*3] = x_pad[:, C//4*2:C//4*3,self.shift*2:, :-self.shift*2]
            x_pad[:, C//4*3:] = x_pad[:, C//4*3:, self.shift*2:, self.shift*2:]
            out = x_pad
