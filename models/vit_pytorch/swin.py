...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size, 
                 ...
                 drop_path=0., norm_layer=nn.LayerNorm, downsample=False, use_checkpoint=False, is_base=True, is_learn=True, n_trans=4, adaptive=False, init_noise=0., eps=0., padding_mode='zeros'):
...
#                self.downsample = ShiftedPatchMerging(dim, dim*2, input_resolution[0])
                ...
                self.downsample = ShiftedPatchTokenization(input_resolution[0]//2 ,dim, dim*2, 2, is_learn=is_learn, init_noise=init_noise, eps=eps, padding_mode=padding_mode)
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 use_checkpoint=False, is_base=True, n_trans=4, is_learn=True, init_noise=0., eps=0., padding_mode='zeros' ,**kwargs):
        super().__init__()
        ...
        if is_base:
            ...
            self.patch_embed = ShiftedPatchTokenization(img_size//patch_size, 3, embed_dim, patch_size, is_learn=is_learn, init_noise=init_noise, eps=eps, padding_mode=padding_mode)
        ...
        for i_layer in range(self.num_layers):
            ...
            layer = BasicLayer(
                               ...
                               use_checkpoint=use_checkpoint, is_base=is_base, is_learn=is_learn, n_trans=n_trans, init_noise=init_noise, eps=eps, padding_mode=padding_mode)
...
class ShiftedPatchTokenization(nn.Module):
    def __init__(self, num_patches, in_dim, dim, merging_size=2, exist_class_t=False, is_learn=False, n_trans=4, init_noise=0., eps=0., padding_mode='zeros'):
        super().__init__()
        ...
        if is_learn:      
            self.patch_shifting = SpatialTransformation_learn(num_patches, init_noise=init_noise, eps=eps, padding_mode=padding_mode)
...
class SpatialTransformation_learn(nn.Module):
    def __init__(self, num_patches, init_noise=0., eps=0., padding_mode='zeros'):
        super().__init__()
