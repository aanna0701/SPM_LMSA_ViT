...
class Transformer(nn.Module):
    def __init__(self, in_dim, dim, num_patches, depth, heads, dim_head, mlp_dim, dropout = 0., layer_dropout = 0., is_LSA=False):
        super().__init__()
    ...
    def forward(self, x, context):
        ...
        for conv, attn, ff in self.layers:
            self.context = conv(self.context)
            x = attn(x, context = self.context) + x
            x = ff(x) + x
        return x  
    ...
    def flops(self):
        ...
        for (_, attn, ff) in self.layers:       
            flops += self.in_dim * self.dim * (3**2) * (self.num_patches//4)
            flops += attn.flops()
            flops += ff.flops()
