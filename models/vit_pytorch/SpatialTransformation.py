...
class AffineNet(nn.Module):
    ...
    def forward(self, param_token, x, init, scale=None):
        ...
        x = self.pre_linear(x)
        chunk = torch.chunk(x, self.n_trans, dim=1)
        for i in range(self.n_trans):
            if scale is not None:
                out.append(self.transformation(chunk[i], param_list[i], init, scale[i]))
            else:
                out.append(self.transformation(chunk[i], param_list[i], init))
...
class PatchMerging(nn.Module):
    ...
    def __init__(self, num_patches, patch_size, dim, out_dim):
        ...
        self.patch_dim = dim * (patch_size ** 2) * 2
