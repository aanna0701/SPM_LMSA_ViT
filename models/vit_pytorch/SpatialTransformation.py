...
class AffineNet(nn.Module):
    def __init__(self, num_patches, depth, in_dim, hidden_dim, heads, n_trans=4, down_sizing=2, is_LSA=False, is_pe=False):
        super().__init__()
        self.in_dim = in_dim
        ...
        if not is_pe:
            ...
            self.patch_conv = nn.Sequential(
                nn.Conv2d(3, self.in_dim, self.down_sizing, self.down_sizing),
                Rearrange('b c h w -> b (h w) c')
            )
    ...
    def forward(self, param_token, x, init, scale=None):
        ...
        param_attd = self.param_transformer(param_token, self.patch_conv(x))
...
class STT(nn.Module):
    def __init__(self, img_size=224, patch_size=2, in_dim=3, embed_dim=96, depth=2, heads=4, type='PE', 
        ...
        if self.type == 'PE':
            if not img_size >= 224:
                if patch_size > 2:   
                    self.affine_net = AffineNet(self.num_patches, depth, self.in_dim, self.in_dim, heads, down_sizing=down_sizing, 
                                                is_LSA=is_LSA, n_trans=n_trans, is_pe=True)
                    self.patch_merge = PatchMerging(self.num_patches, patch_size, 3*(n_trans+1), embed_dim) 
                else:    
                    self.affine_net = AffineNet(self.num_patches, depth, self.in_dim, self.in_dim, heads, down_sizing=down_sizing, 
                                                is_LSA=is_LSA, n_trans=n_trans, is_pe=True)
                    self.patch_merge = PatchMerging(self.num_patches, patch_size, 3*(n_trans+1), embed_dim) 
            ...
            self.patch_merge = PatchMerging(self.num_patches, 2, self.in_dim*(n_trans+1), self.in_dim*2)
